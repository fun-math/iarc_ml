...
def callback(data):
    ...
    amp=2
    R=amp*np.sin(dt)
    r=0#np.pi/6*np.sin(np.sqrt(7)/2*dt)
    p=0
    y=0
    ...
    state_msg.pose.position.x = 4.4-X#10-amp+R#4.1-x*ct+z*st#4.3-amp+R
    state_msg.pose.position.y = Y#(amp-R)/2*np.cos(np.e/2*dt)#-0.25+Y#-0.05+(amp-R)/2*np.cos(np.e/2*dt)
    state_msg.pose.position.z = 1.6#(amp-R)/4*np.sin(np.sqrt(5)*dt)#1.51+x*st+z*ct#1.2+(amp-R)/4*np.sin(np.sqrt(5)*dt)
